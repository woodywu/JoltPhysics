...
public:
    ...
    inline fixed& operator*=(const fixed& y) noexcept
    {
        ...
        auto final_value = (value / 2) + (value % 2);
        assert(final_value < std::numeric_limits<BaseType>::max());
        assert(final_value > std::numeric_limits<BaseType>::min());
        m_value = static_cast<BaseType>(final_value);
        return *this;
    }
...
template <typename B, typename I, unsigned int F>
constexpr std::float_denorm_style numeric_limits<fpm::fixed<B,I,F>>::has_denorm;
template <typename B, typename I, unsigned int F>
