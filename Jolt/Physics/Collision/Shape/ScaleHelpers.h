...
namespace ScaleHelpers
{
	...
	static constexpr decimal	cScaleToleranceSq = decimal(1.0e-8f);

	/// Test if a scale is identity
	inline bool				IsNotScaled(Vec3Arg inScale)									{ return inScale.IsClose(Vec3::sReplicate(C1), cScaleToleranceSq); }
	...
	inline decimal			ScaleConvexRadius(decimal inConvexRadius, Vec3Arg inScale)		{ return min(inConvexRadius * inScale.Abs().ReduceMin(), cDefaultConvexRadius); }
	...
	inline Vec3				MakeUniformScale(Vec3Arg inScale)								{ return Vec3::sReplicate((inScale.GetX() + inScale.GetY() + inScale.GetZ()) / C3); }
	...
	inline bool				CanScaleBeRotated(QuatArg inRotation, Vec3Arg inScale)
	{
		...
		Vec4 epsilon = Vec4::sReplicate(decimal(1.0e-6f));
