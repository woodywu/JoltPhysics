...
public:
	...
	Vec3					mPosition { C0, C0, C0 };						///< Attachment point of wheel suspension in local space of the body
	Vec3					mDirection { C0, -C1, C0 };					///< Direction of the suspension in local space of the body
	decimal					mSuspensionMinLength = decimal(0.3f);				///< How long the suspension is in max raised position relative to the attachment point (m)
	decimal					mSuspensionMaxLength = decimal(0.5f);				///< How long the suspension is in max droop position relative to the attachment point (m)
	decimal					mSuspensionPreloadLength = decimal(0.0f);			///< The natural length (m) of the suspension spring is defined as mSuspensionMaxLength + mSuspensionPreloadLength. Can be used to preload the suspension as the spring is compressed by mSuspensionPreloadLength when the suspension is in max droop position. Note that this means when the vehicle touches the ground there is a discontinuity so it will also make the vehicle more bouncy as we're updating with discrete time steps.
	decimal					mSuspensionFrequency = decimal(1.5f);				///< Natural frequency of the suspension spring (Hz)
	decimal					mSuspensionDamping = decimal(0.5f);					///< Damping factor of the suspension spring (0 = no damping, 1 = critical damping)
	decimal					mRadius = decimal(0.3f);								///< Radius of the wheel (m)
	decimal					mWidth = decimal(0.1f);								///< Width of the wheel (m)
};
...
public:
	...
	decimal					GetAngularVelocity() const					{ return mAngularVelocity; }

	/// Update the angular velocity (rad/s)
	void					SetAngularVelocity(decimal inVel)				{ mAngularVelocity = inVel; }

	/// Get the current rotation angle of the wheel in radians [0, 2 pi]
	decimal					GetRotationAngle() const					{ return mAngle; }

	/// Set the current rotation angle of the wheel in radians [0, 2 pi]
	void					SetRotationAngle(decimal inAngle)				{ mAngle = inAngle; }

	/// Get the current steer angle of the wheel in radians [-pi, pi], positive is to the left
	decimal					GetSteerAngle() const						{ return mSteerAngle; }

	/// Set the current steer angle of the wheel in radians [-pi, pi]
	void					SetSteerAngle(decimal inAngle)				{ mSteerAngle = inAngle; }
	...
	decimal					GetSuspensionLength() const					{ return mContactLength - mSettings->mRadius; }
	...
	decimal					GetSuspensionLambda() const					{ return mSuspensionPart.GetTotalLambda() + mSuspensionMaxUpPart.GetTotalLambda(); }

	/// Get total impulse (N s) applied along the forward direction of the wheel
	decimal					GetLongitudinalLambda() const				{ return mLongitudinalPart.GetTotalLambda(); }

	/// Get total impulse (N s) applied along the sideways direction of the wheel
	decimal					GetLateralLambda() const					{ return mLateralPart.GetTotalLambda(); }
	...
	bool					SolveLongitudinalConstraintPart(const VehicleConstraint &inConstraint, decimal inMinImpulse, decimal inMaxImpulse);
	...
	bool					SolveLateralConstraintPart(const VehicleConstraint &inConstraint, decimal inMinImpulse, decimal inMaxImpulse);
...
protected:
	...
	decimal					mContactLength;								///< Length between attachment point and ground
	...
	decimal					mAntiRollBarImpulse = decimal(0.0f);					///< Amount of impulse applied to the suspension from the anti-rollbars

	decimal					mSteerAngle = decimal(0.0f);							///< Rotation around the suspension direction, positive is to the left
	decimal					mAngularVelocity = decimal(0.0f);					///< Rotation speed of wheel, positive when the wheels cause the vehicle to move forwards (rad/s)
	decimal					mAngle = decimal(0.0f);								///< Current rotation of the wheel (rad, [0, 2 pi])
