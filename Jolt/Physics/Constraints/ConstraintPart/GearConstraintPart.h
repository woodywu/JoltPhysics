...
public:
	...
	inline void					CalculateConstraintProperties(const Body &inBody1, Vec3Arg inWorldSpaceHingeAxis1, const Body &inBody2, Vec3Arg inWorldSpaceHingeAxis2, decimal inRatio)
	{
		JPH_ASSERT(inWorldSpaceHingeAxis1.IsNormalized(decimal(1.0e-4f)));
		JPH_ASSERT(inWorldSpaceHingeAxis2.IsNormalized(decimal(1.0e-4f)));
		...
		mEffectiveMass = decimal(1.0f) / (inWorldSpaceHingeAxis1.Dot(mInvI1_A) + inWorldSpaceHingeAxis2.Dot(mInvI2_B) * Square(inRatio));
	}
	...
	/// @param inWorldSpaceHingeAxis2 The axis around which body 2 rotates
	/// @param inRatio The ratio between rotation and translation
	inline bool					SolveVelocityConstraint(Body &ioBody1, Vec3Arg inWorldSpaceHingeAxis1, Body &ioBody2, Vec3Arg inWorldSpaceHingeAxis2, decimal inRatio)
	{
		// Lagrange multiplier is:
		//
		// lambda = -K^-1 (J v + b)
		decimal lambda = -mEffectiveMass * (inWorldSpaceHingeAxis1.Dot(ioBody1.GetAngularVelocity()) + inRatio * inWorldSpaceHingeAxis2.Dot(ioBody2.GetAngularVelocity()));
