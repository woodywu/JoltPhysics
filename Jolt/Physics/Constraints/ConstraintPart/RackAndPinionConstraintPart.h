...
public:
	...
	inline void					CalculateConstraintProperties(const Body &inBody1, Vec3Arg inWorldSpaceHingeAxis, const Body &inBody2, Vec3Arg inWorldSpaceSliderAxis, decimal inRatio)
	{
		JPH_ASSERT(inWorldSpaceHingeAxis.IsNormalized(decimal(1.0e-4f)));
		JPH_ASSERT(inWorldSpaceSliderAxis.IsNormalized(decimal(1.0e-4f)));

		// Calculate: I1^-1 a
		mInvI1_A = inBody1.GetMotionProperties()->MultiplyWorldSpaceInverseInertiaByVector(inBody1.GetRotation(), inWorldSpaceHingeAxis);

		// Calculate: r/m2 b
		decimal inv_m2 = inBody2.GetMotionProperties()->GetInverseMass();
		mRatio_InvM2_B = inRatio * inv_m2 * inWorldSpaceSliderAxis;

		// K^-1 = 1 / (J M^-1 J^T) = 1 / (a^T I1^-1 a + 1/m2 * r^2 * b . b)
		mEffectiveMass = decimal(1.0f) / (inWorldSpaceHingeAxis.Dot(mInvI1_A) + inv_m2 * Square(inRatio));
	}
	...
	/// @param inWorldSpaceSliderAxis The axis along which body 2 slides
	/// @param inRatio The ratio between rotation and translation
	inline bool					SolveVelocityConstraint(Body &ioBody1, Vec3Arg inWorldSpaceHingeAxis, Body &ioBody2, Vec3Arg inWorldSpaceSliderAxis, decimal inRatio)
	{
		// Lagrange multiplier is:
		//
		// lambda = -K^-1 (J v + b)
		decimal lambda = mEffectiveMass * (inRatio * inWorldSpaceSliderAxis.Dot(ioBody2.GetLinearVelocity()) - inWorldSpaceHingeAxis.Dot(ioBody1.GetAngularVelocity()));
