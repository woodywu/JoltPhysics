...
public:
	...
	void								DrawMarker(RVec3Arg inPosition, ColorArg inColor, decimal inSize);

	/// Draw an arrow
	void								DrawArrow(RVec3Arg inFrom, RVec3Arg inTo, ColorArg inColor, decimal inSize);

	/// Draw coordinate system (3 arrows, x = red, y = green, z = blue)
	void								DrawCoordinateSystem(RMat44Arg inTransform, decimal inSize = C1);

	/// Draw a plane through inPoint with normal inNormal
	void								DrawPlane(RVec3Arg inPoint, Vec3Arg inNormal, ColorArg inColor, decimal inSize);
	...
	/// Draw a sphere
	void								DrawSphere(RVec3Arg inCenter, decimal inRadius, ColorArg inColor, ECastShadow inCastShadow = ECastShadow::On, EDrawMode inDrawMode = EDrawMode::Solid);
	...
	void								DrawCapsule(RMat44Arg inMatrix, decimal inHalfHeightOfCylinder, decimal inRadius, ColorArg inColor, ECastShadow inCastShadow = ECastShadow::On, EDrawMode inDrawMode = EDrawMode::Solid);
	...
	void								DrawCylinder(RMat44Arg inMatrix, decimal inHalfHeight, decimal inRadius, ColorArg inColor, ECastShadow inCastShadow = ECastShadow::On, EDrawMode inDrawMode = EDrawMode::Solid);
	...
	void								DrawOpenCone(RVec3Arg inTop, Vec3Arg inAxis, Vec3Arg inPerpendicular, decimal inHalfAngle, decimal inLength, ColorArg inColor, ECastShadow inCastShadow = ECastShadow::On, EDrawMode inDrawMode = EDrawMode::Solid);
	...
	/// @param inCastShadow determins if this geometry should cast a shadow or not.
	/// @param inDrawMode determines if we draw the geometry solid or in wireframe.
	void								DrawSwingLimits(RMat44Arg inMatrix, decimal inSwingYHalfAngle, decimal inSwingZHalfAngle, decimal inEdgeLength, ColorArg inColor, ECastShadow inCastShadow = ECastShadow::On, EDrawMode inDrawMode = EDrawMode::Solid);
	...
	/// @param inCastShadow determins if this geometry should cast a shadow or not.
	/// @param inDrawMode determines if we draw the geometry solid or in wireframe.
	void								DrawPie(RVec3Arg inCenter, decimal inRadius, Vec3Arg inNormal, Vec3Arg inAxis, decimal inMinAngle, decimal inMaxAngle, ColorArg inColor, ECastShadow inCastShadow = ECastShadow::On, EDrawMode inDrawMode = EDrawMode::Solid);
	...
	public:
		...
		/// Constructor
										...
										Geometry(const Batch &inBatch, const AABox &inBounds) : mBounds(inBounds) { mLODs.push_back({ inBatch, FIX_MAX }); }
	...
	/// @param inDrawMode determines if we draw the geometry solid or in wireframe.
	virtual void						DrawGeometry(RMat44Arg inModelMatrix, const AABox &inWorldSpaceBounds, decimal inLODScaleSq, ColorArg inModelColor, const GeometryRef &inGeometry, ECullMode inCullMode = ECullMode::CullBackFace, ECastShadow inCastShadow = ECastShadow::On, EDrawMode inDrawMode = EDrawMode::Solid) = 0;
	...
	/// Draw text
	virtual void						DrawText3D(RVec3Arg inPosition, const string_view &inString, ColorArg inColor = Color::sWhite, decimal inHeight = C0P5) = 0;
...
private:
	...
	struct SwingLimits
	{
		...
		decimal							mSwingYHalfAngle;
		decimal							mSwingZHalfAngle;
	};
	...
	using PieBatces = UnorderedMap<decimal, GeometryRef>;
