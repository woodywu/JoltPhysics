...
public:
	...
	JPH_INLINE					Vec4(Vec3Arg inRHS, decimal inW);
	JPH_INLINE					Vec4(Type inRHS) : mValue(inRHS)				{ }

	/// Create a vector from 4 components
	JPH_INLINE					Vec4(decimal inX, decimal inY, decimal inZ, decimal inW);
	...
	static JPH_INLINE Vec4		sReplicate(decimal inV);

	/// Load 4 decimals from memory
	static JPH_INLINE Vec4		sLoadFloat4(const Float4 *inV);

	/// Load 4 decimals from memory, 16 bytes aligned
	static JPH_INLINE Vec4		sLoadFloat4Aligned(const Float4 *inV);

	/// Gather 4 decimals from memory at inBase + inOffsets[i] * Scale
	template <const int Scale>
	static JPH_INLINE Vec4		sGatherFloat4(const decimal *inBase, UVec4Arg inOffsets);
	...
	JPH_INLINE decimal			GetW() const									{ return mF32[3]; }

	/// Set individual components
	JPH_INLINE void				SetX(decimal inX)									{ mF32[0] = inX; }
	JPH_INLINE void				SetY(decimal inY)									{ mF32[1] = inY; }
	JPH_INLINE void				SetZ(decimal inZ)									{ mF32[2] = inZ; }
	JPH_INLINE void				SetW(decimal inW)									{ mF32[3] = inW; }

	/// Get decimal component by index
	JPH_INLINE decimal			operator [] (uint inCoordinate) const			{ JPH_ASSERT(inCoordinate < 4); return mF32[inCoordinate]; }
	JPH_INLINE decimal &			operator [] (uint inCoordinate)					{ JPH_ASSERT(inCoordinate < 4); return mF32[inCoordinate]; }
	...
	JPH_INLINE bool				IsClose(Vec4Arg inV2, decimal inMaxDistSq = decimal(1.0e-12f)) const;
	...
	JPH_INLINE Vec4				operator * (decimal inV2) const;

	/// Multiply vector with decimal
	friend JPH_INLINE Vec4		operator * (decimal inV1, Vec4Arg inV2);

	/// Divide vector by decimal
	JPH_INLINE Vec4				operator / (decimal inV2) const;

	/// Multiply vector with decimal
	JPH_INLINE Vec4 &			operator *= (decimal inV2);
	...
	/// Dot product
	JPH_INLINE decimal			Dot(Vec4Arg inV2) const;
