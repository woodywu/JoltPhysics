...
JPH_INLINE decimal RaySphere(Vec3Arg inRayOrigin, Vec3Arg inRayDirection, Vec3Arg inSphereCenter, decimal inSphereRadius)
{
	...
	decimal a = inRayDirection.LengthSq();
	decimal b = C2 * inRayDirection.Dot(center_origin);
	decimal c = center_origin.LengthSq() - inSphereRadius * inSphereRadius;
	decimal fraction1, fraction2;
	if (FindRoot(a, b, c, fraction1, fraction2) == 0)
		return c <= C0? C0 : FIX_MAX; // Return if origin is inside the sphere
	...
	if (fraction2 >= C0)
		return C0; // We start inside the sphere
...
JPH_INLINE int RaySphere(Vec3Arg inRayOrigin, Vec3Arg inRayDirection, Vec3Arg inSphereCenter, decimal inSphereRadius, decimal &outMinFraction, decimal &outMaxFraction)
{
	// Solve: |RayOrigin + fraction * RayDirection - SphereCenter|^2 = SphereRadius^2 for fraction
	Vec3 center_origin = inRayOrigin - inSphereCenter;
	decimal a = inRayDirection.LengthSq();
	...
	case 0:
		if (c <= C0)
		{
			...
			outMinFraction = outMaxFraction = C0;
			return 1;
		}
		else
		{
