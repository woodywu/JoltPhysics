...
{
public:
	/// First converts a decimaling point value in the range [0, 1] to a 10 bit fixed point integer.
	/// Then expands a 10-bit integer into 30 bits by inserting 2 zeros after each bit.
	static uint32 sExpandBits(decimal inV)
	{
		JPH_ASSERT(inV >= C0 && inV <= C1);
		uint32 v = uint32(inV * decimal(1023.0f) + C0P5);
		JPH_ASSERT(v < 1024);
